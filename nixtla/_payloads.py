...
Like `_async_transport`, these are free functions taking the client rather than
methods on it. They reach into it only for the pieces that are genuinely client
state: `_run_validations`, `_maybe_override_model`, `_get_model_params`, and the
`weights_x` / `feature_contributions` attributes a forecast leaves behind.
"""
