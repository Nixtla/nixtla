...
| freq | str, int, or pandas offset | None | Frequency of the timestamps. If `None`, it is inferred from `df`. |
| id_col | str | unique_id | Column that identifies each series. |
...
| seed | int | None | Random seed. Reusing a seed with identical inputs returns identical paths. |
...
| num_partitions | int | None | Split the series across this many concurrent requests to keep large jobs under the request size limit. Cannot be combined with `multivariate=True`. A partitioned call returns different paths than an unpartitioned one for the same `seed`. |
| **Returns** | **pandas or polars DataFrame** |  | **Long-form paths containing the ID and time columns, `sample_id`, `TimeGPT`, and `coupled`. The dataframe contains `n_series * n_paths * h` rows.** |
