...
When using Azure AI, `finetune_steps` works as described above: it fine-tunes the model at inference time using your data. However, fine-tuned model persistence (saving and reusing models with `NixtlaClient.finetune` and `finetuned_model_id`) is not available for Azure AI deployments. For more details, see [Re-using fine-tuned models](/forecasting/fine-tuning/save_reuse_delete_finetuned_models).
