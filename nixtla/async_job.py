...
class AsyncJobError(RuntimeError):
    """Raised when a server-side async job ends without a result.
    ...
    Attributes:
        ...
        status (str, optional): Terminal status reported by the server,
            `"failed"` or `"cancelled"`; set together with `task`.
    """
    ...
    def __str__(self) -> str:
        if self.task is not None:
            ...
            return f"{self.task} job '{self.job_id}' {self.status}: {detail}"
