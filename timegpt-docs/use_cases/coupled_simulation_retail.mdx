...
<Note>
The two runs use different seeds. With the same seed both shuffles draw their
template windows from the same generator state, so the first series by name would
receive an identical set of paths in both runs.
</Note>
