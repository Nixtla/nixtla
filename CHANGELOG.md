...
- **TSMP steps.** `submit_execute_step_job()` runs one TSMP top-level API call server-side, returning a `Job` whose `wait()` gives back a `StepResult`. Tables are referenced from `params` with `nixtla.ref(key)`, and one step's `data` can be passed straight into the next, so calls chain without any file or byte handling.
