...
every 15 seconds -- and waits up to an hour per job; pass `poll_interval` for a
fixed cadence or `poll_timeout` for a different ceiling, or use
`NixtlaClient.jobs.simulate()` to get a [`Job` handle](/jobs/asynchronous_jobs)
back immediately and wait on it yourself. A job that
fails on the server raises `nixtla.JobError` with the server's original error. Server-side cancellation raises
