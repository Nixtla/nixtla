...
cross_validation(
    ...
    model_parameters: Annotated[Optional[Dict[str, Any]], AfterValidator(func=<function validate_extra_params at 0x7f014590d6c0>)] = None,
...
**Args:**
 ...
 - <b>`multivariate`</b> (bool):  If True, enables multivariate predictions. 
 - <b>`Defaults to False. Note`</b>:  multivariate predictions are only supported for a select set of TimeGPT models.  
...
**Args:**
 ...
 - <b>`multivariate`</b> (bool):  If True, enables multivariate predictions. 
 - <b>`Defaults to False. Note`</b>:  multivariate predictions are only supported for a select set of TimeGPT models. This variable  is different from the `threshold_method` parameter. The latter controls the method used for anomaly detection (univariate vs multivariate) whereas `multivariate` determines how the model  creates the predictions. 
