...
Required environment variables:
    ...
    NIXTLA_API_KEY_CUSTOM: Nixtla API key for authentication
...
def test_config() -> SnowflakeTestConfig:
    ...
    """
    api_key = os.getenv("NIXTLA_API_KEY_CUSTOM")

    if not api_key:
        pytest.skip("NIXTLA_API_KEY_CUSTOM, skipping Snowflake tests")

    assert isinstance(api_key, str) and len(api_key) > 0, (
        "NIXTLA_API_KEY_CUSTOM must be a non-empty string"
    )
