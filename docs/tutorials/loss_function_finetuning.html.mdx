...
function to the `fewshot_loss` parameter of the `forecast` method.
...
```python
timegpt_fcst_fewshot_mae_df = timegpt.forecast(
    df=df, 
    h=12, 
    fewshot_steps=10,
    fewshot_loss='mae',   # Set your desired loss function
    time_col='timestamp', 
    target_col='value',
)
```
...
```python
timegpt.plot(
    df, timegpt_fcst_fewshot_mae_df, 
