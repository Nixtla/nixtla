...
- **Recovering a job.** `client.jobs.list()` lists your team's jobs newest first, defaulting to the pending and running ones and paging through the rest internally; pass `status`, `task` or `limit` to narrow it. `client.jobs.retrieve(job_id)` turns an id back into a working `Job` — `status`, `wait()`, `cancel()` and the context manager all behave as they do on a submitted one — which is how a job outlives the process that started it. A retrieved job's `wait()` returns the server's **raw result dict** rather than the dataframe the original call would have built: the series ids and column names are never sent to the server, so they cannot come back with the result. Values run in sorted series-id order, grouped by the `sizes` the task reports, so a caller still holding the input frame can put the labels back on. `execute_step` is the exception — its result carries its own schema, so `wait()` returns the usual `StepResult`. `JobSummary` is exported from the package root.
