...
The accepted range is 1 to 10,000 paths. A request can return at most 5,000,000
values:

```text
n_paths * number of series * h <= 5,000,000
```

For example, 500 series, a horizon of 20, and 500 paths would produce exactly
5,000,000 values.

A second limit applies when you also pass a [quantile grid](#quantile-grid),
because the service builds those marginals alongside the paths:

```text
number of series * h * (n_paths + len(quantiles)) <= 10,000,000
```

Both limits are checked before the request is sent, so an oversized call fails
immediately rather than after a long wait.
...
A wide grid also counts towards the second
[size limit](#choose-the-number-of-paths): `number of series * h * (n_paths +
len(quantiles))` may not exceed 10,000,000.
