...
- **Asynchronous jobs.** Long-running work can now run server-side without blocking the client, through the `client.jobs` namespace: `jobs.forecast()`, `jobs.cross_validation()`, `jobs.finetune()`, `jobs.detect_anomalies()`, `jobs.simulate()`, `jobs.explain()` and `jobs.execute_step()` return a `Job` handle instead of a result. `job.wait()` blocks until it finishes, `job.status` reports progress, and `job.cancel()` asks the server to stop. Each method mirrors the blocking method of the same name, so the async name is derivable from the sync one — note that `jobs.detect_anomalies()` mirrors `detect_anomalies_online()`, i.e. it runs *online* detection. `job.wait()` polls on an adaptive cadence by default — the first status check comes after half a second and the interval doubles up to one check every 15 seconds — giving up after an hour; pass `poll_interval` for a fixed cadence instead, or `poll_timeout=None` to wait until the server reports a terminal status. `simulate()` and `explain()` take the same two arguments, and `job_timeout_seconds` caps the time the server itself spends. A `Job` used as a context manager cancels the job if an exception leaves the block while it is still running. `Job`, `JobStatus`, `JobError`, `JobTimeoutError`, `JobCancelledError` and `ApiError` are exported from the package root.
- **Recovering a job.** `client.jobs.list()` lists your team's jobs newest first, defaulting to the pending and running ones and paging through the rest internally; pass `status` (one status or a list), `task` or `limit` to narrow it — `limit` bounds the rows fetched, so with `task` set, which filters client-side, it can return fewer matches than exist further down. A row whose status this SDK does not model is skipped with a warning rather than failing the call. `client.jobs.retrieve(job_id)` turns an id back into a working `Job` — `status`, `wait()`, `cancel()` and the context manager all behave as they do on a submitted one — so a job can outlive the process that started it. A retrieved job's `wait()` returns the server's **raw result dict** rather than a parsed dataframe: the series ids and column names never reach the server, so they cannot come back. `execute_step` is the exception, its result being self-describing. `JobSummary` is exported from the package root.
...
- **TSMP steps.** `jobs.execute_step()` runs one TSMP top-level API call server-side, returning a `Job` whose `wait()` gives back a `StepResult`. Tables are referenced from `params` with `nixtla.ref(key)`, and one step's `data` can be passed straight into the next, so calls chain without any file or byte handling.
