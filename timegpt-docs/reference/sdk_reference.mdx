...
>     feature_contributions_type="shapley", model_parameters=None,
>     multivariate=False
> )
> ```
...
> ```text
> NixtlaClient.explain (
>     df, method="granger", features=None, id_col="unique_id",
>     time_col="ds", target_col="y", categorical_exog_list=None,
...
| features | list[str] | None | Features to analyze. By default, every column other than the ID, time, and target columns is used. |
| id_col | str | unique_id | Column that identifies each series. |
| time_col | str | ds | Column that identifies each timestep. |
| target_col | str | y | Column that contains the target. |
