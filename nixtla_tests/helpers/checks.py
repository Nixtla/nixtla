...
def check_quantiles(nixtla_client: NixtlaClient, df: fugue.AnyDataFrame, id_col: str = 'id_col', time_col: str = 'time_col'):
...
def check_forecast_diff_results_diff_models(
    ...
    **fcst_kwargs,
    ...
    with pytest.raises(AssertionError, match=r'\(column name="TimeGPT"\) are different') as exc_info:
        pd.testing.assert_frame_equal(
            fcst_df.sort_values([id_col, time_col]).reset_index(drop=True),
            fcst_df_2.sort_values([id_col, time_col]).reset_index(drop=True),
        )
...
def check_forecast_dataframe(
    ...
    check_cv_same_results_num_partitions(nixtla_client, df, n_windows=3, step_size=None, horizon=1)
    check_cv_same_results_num_partitions(nixtla_client, df, model='timegpt-1-long-horizon', horizon=1)
    check_forecast_diff_results_diff_models(nixtla_client, df)
    check_forecast(nixtla_client, df, num_partitions=1)
    check_forecast(nixtla_client, df, level=[90, 80], num_partitions=1, n_series_to_check=n_series_to_check)
...
def check_forecast_dataframe_diff_cols(
    ...
    check_forecast(
        nixtla_client, df, id_col=id_col, time_col=time_col, target_col=target_col, num_partitions=1,
    )
    check_forecast(
        nixtla_client, df, id_col=id_col, time_col=time_col, target_col=target_col, level=[90, 80],  num_partitions=1,
    )
...
def check_anomalies_online_dataframe(nixtla_client: NixtlaClient, df: fugue.AnyDataFrame):
    check_online_anomalies(nixtla_client, df, h=20, detection_size=5, threshold_method='univariate', level=99, num_partitions=1)
    check_anomalies_online_same_results_num_partitions(nixtla_client, df, h=20, detection_size=5, threshold_method='univariate', level=99)
...
def check_anomalies_dataframe_diff_cols(
    ...
    check_anomalies(nixtla_client, df, id_col=id_col, time_col=time_col, target_col=target_col, num_partitions=1)
    check_anomalies(nixtla_client, df, id_col=id_col, time_col=time_col, target_col=target_col, level=90, num_partitions=1)
    check_anomalies_same_results_num_partitions(nixtla_client, df, id_col=id_col, time_col=time_col, target_col=target_col)
...
def check_forecast_x_dataframe(nixtla_client: NixtlaClient, df: fugue.AnyDataFrame, X_df: fugue.AnyDataFrame):
...
def check_forecast_x_dataframe_diff_cols(
    ...
    check_forecast_x(
        nixtla_client, df, X_df, id_col=id_col, time_col=time_col, target_col=target_col, num_partitions=1
    )
    check_forecast_x(
        nixtla_client, df, X_df, id_col=id_col, time_col=time_col, target_col=target_col, level=[90, 80], num_partitions=1
    )
...
def check_finetuned_model(
    ...
    with pytest.raises(ValueError, match='Can only fine-tune on pandas or polars dataframes.'):
    ...
    distr_fcst = fa.as_pandas(
        nixtla_client.forecast(df=df, h=5, finetuned_model_id=model_id2)
    ...
    distr_cv = fa.as_pandas(
        nixtla_client.cross_validation(df=df, n_windows=2, h=5, finetuned_model_id=model_id2)
    ).sort_values(['unique_id', 'ds']).reset_index(drop=True)
    pd.testing.assert_frame_equal(
        local_cv,
