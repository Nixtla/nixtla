...
- **Recovering a job.** `client.jobs.list()` lists your team's jobs newest first, defaulting to the pending and running ones and paging through the rest internally; pass `status` (one status or a list), `task` or `limit` to narrow it — `limit` bounds the rows fetched, so with `task` set, which filters client-side, it can return fewer matches than exist further down. `client.jobs.retrieve(job_id)` turns an id back into a working `Job` — `status`, `wait()`, `cancel()` and the context manager all behave as they do on a submitted one — so a job can outlive the process that started it. A retrieved job's `wait()` returns the server's **raw result dict** rather than a parsed dataframe: the series ids and column names never reach the server, so they cannot come back. `execute_step` is the exception, its result being self-describing. `JobSummary` is exported from the package root.
