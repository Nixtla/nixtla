...
The `submit_*_job` methods below submit the same work without waiting for it,
...
behavior, or `NixtlaClient.submit_simulate_job` to get a `Job` handle back
immediately. A job that fails on the server
raises `nixtla.AsyncJobError`. If the job outlives `poll_timeout`,
the client requests cancellation and raises `nixtla.AsyncJobTimeoutError`.
Server-side cancellation raises `nixtla.AsyncJobCancelledError`. Invalid inputs,
...
behavior, or `NixtlaClient.submit_explain_job` to get a `Job` handle back
immediately. A job that fails on the server
raises `nixtla.AsyncJobError`. If the job outlives `poll_timeout`,
the client requests cancellation and raises `nixtla.AsyncJobTimeoutError`.
Server-side cancellation raises `nixtla.AsyncJobCancelledError`. Invalid inputs,
...
`submit_*_job` methods below send the same request their blocking counterparts
...
Returned by every `submit_*_job` method; not constructed directly. Importable
...
| poll_timeout | float | 3600.0 | Maximum seconds to wait for a terminal state before raising `AsyncJobTimeoutError`. Must be finite and positive, or `None` to poll until the server reports a terminal status. |
...
Raises `AsyncJobError` if the job fails server-side, `AsyncJobCancelledError` if
it reaches the `cancelled` state, and `AsyncJobTimeoutError` if `poll_timeout`
...
```python
with client.submit_forecast_job(df=df, h=12, freq="D") as job:
    fcst = job.wait()
```
