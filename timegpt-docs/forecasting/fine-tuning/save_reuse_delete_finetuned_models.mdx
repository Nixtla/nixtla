...
<Warning>
  Fine-tuned model persistence (saving and reusing fine-tuned models) is **only available** when using the public TimeGPT API. This feature is **not available** for Azure AI deployments. When using Azure AI, you can still use the `finetune_steps` parameter in the `forecast` method to apply fine-tuning at inference time, but the fine-tuned model will not be saved for future reuse.
</Warning>
