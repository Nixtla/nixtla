...
`explain()`, or `Job.wait()`. They default to a status check every 15 seconds
and a one-hour ceiling; `poll_timeout=None` waits until the server reports a
terminal status.
...
>     job_timeout_seconds=None, poll_interval=15.0, poll_timeout=3600.0
> )
> ```
...
| poll_interval | float | 15.0 | Seconds between two status checks of the job. |
