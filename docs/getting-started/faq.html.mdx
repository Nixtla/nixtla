...
to produce accurate forecasts. Using less data may require fine-tuning
and could lead to wider prediction intervals. While `TimeGPT` can work
with just one observation, its accuracy will be limited.
...
One token corresponds to one row of your dataset. Please refer our
tutorials to understand the data format required for each use case. For
high-frequency data, we recommend trimming the input whenever possible.
For example, when working with minute-level data, consider trimming to
the most recent hour to reduce costs.
