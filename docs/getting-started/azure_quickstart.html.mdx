...
```python
timegen_fcst_df = nixtla_client.forecast(df=df, h=12, freq='MS', time_col='timestamp', target_col='value', model="azureai")
...
```python
timegen_fcst_df = nixtla_client.forecast(df=df, h=36, time_col='timestamp', target_col='value', freq='MS', model='timegpt-1-long-horizon')
timegen_fcst_df.head()
```
...
the default Azure TimeGEN-1 model, `azureai`.

```python
timegen_fcst_df = nixtla_client.forecast(df=df, h=6, time_col='timestamp', target_col='value', freq='MS', model="azureai")
nixtla_client.plot(df, timegen_fcst_df, time_col='timestamp', target_col='value')
```
