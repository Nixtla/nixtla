...
blocks like any other client method. By default the client checks every 15
seconds and waits up to an hour; pass `poll_interval` and `poll_timeout` if you
need a different behavior, or use `NixtlaClient.submit_explain_job()` to get a
