...
A promotion rarely affects just one product. Lowering one price can:

- Increase sales of the promoted product
- Reduce sales of an alternative product through **cannibalization**
- Increase sales of a related product through **cross-selling**
...
Those changes also need to be planned together. A retailer may need to know
whether several products will run high on the same day, not only how many units
each product may sell over the month.

This tutorial uses coupled simulation to answer both questions:
...
## The example product group

The data is generated, not observed. That is deliberate: calling one product a
substitute and another a complement is only honest if those relationships are
actually there. Here they are written into the generator, so the labels are true
by construction and the simulation's answers can be checked against them.
...
through December 30, 2025. Every row carries all three prices. This is
important: when the promoted product's price changes, the demand simulation for
every product can respond.
...
Because footfall is shared, the products' demands genuinely move together. Their
rank correlations over the history are 0.49 between the promoted and alternative
products, 0.88 between the promoted and related products, and 0.47 between the
alternative and related products.
...
Generate 500 possible futures for all three products:
...
Run the same request with the discounted price:
...
## Measure cannibalization and cross-selling
...
| Promoted product | 1,102.3 | 1,167.8 | +65.6 | +6.0% |
| Related product | 788.1 | 830.6 | +42.4 | +5.4% |

Every direction matches the planted structure. The promoted product gains
approximately 66 units. The related product gains approximately 42 units, which
is the cross-selling signal. The alternative product loses approximately 179
units, which is the cannibalization signal.

<Note>
Compare these with the planted effects above. The cannibalization comes back
almost exactly (−11.7% against a planted −11.5%), while the two gains come back
at roughly half their planted size (+6.0% against +11.9%, and +5.4% against
+15.0%). Forecasting a price level the model has not seen is conservative by
nature, so treat these as directionally right and, for the gains, understated —
not as exact elasticities.
</Note>
...
## Why simulate the products together?

To see what this changes, generate the promotion paths once more with the
products simulated separately:
...
future for the complete product group, and the shared footfall factor reappears
as products that are busy together. Down the left-hand column the same colors do
not line up — blue is among the lowest paths for the promoted product and the
highest for the alternative one — so no single path there describes the group.

That is what makes questions about shared inventory and staffing possible.

<Note>
The two runs use different seeds on purpose. With one seed, the coupled and the
per-series shuffle pick their template windows from the same random state, so the
first product by name is reordered identically in both runs — for this data it
comes back with exactly the same 500 paths, which would make one row of the chart
above appear twice.
</Note>
