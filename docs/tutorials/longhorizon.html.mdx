...
long-horizon forecasting by specifying `model='long-horizon'` in your
setup.
...
need to set the `model` parameter to `"long-horizon"`. This is the
specialized model in TimeGPT that can handle such tasks.
