...
`NixtlaClient.submit_simulate_job()` to get a `Job` handle back immediately and
wait on it yourself. A job that
fails on the server raises `nixtla.AsyncJobError` with the server's original error. Server-side cancellation raises
