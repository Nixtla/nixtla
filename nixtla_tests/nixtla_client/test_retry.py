...
@pytest.mark.parametrize(
        ...
        [(raise_api_error_with_text, ApiError, True), (raise_api_error_with_json, ApiError, False)]
)
def test_retry_behavior(air_passengers_df, side_effect, side_effect_exception, should_retry):
...
side_effect_settings = [(raise_read_timeout_error, httpx.ReadTimeout), (raise_http_error, ApiError)]

@pytest.mark.parametrize(
    "retry_settings,side_effect_settings",
    product(combs, side_effect_settings)
)
